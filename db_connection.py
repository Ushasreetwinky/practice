...
def retrieve_username(ID):
    db=sqlite3.connect('Twitter.db')
    cursor=db.cursor()
    query="select username from login_details where id="+str(ID)
...
def retrieve_followers(ID):
    ...
    followers=[]
    for row in cursor.execute(query):
        followers.append(retrieve_username(row[0]))
    return followers
...
def retrieve_following(ID):
    ...
    following=[]
    for row in cursor.execute(query):
        following.append(retrieve_username(row[0]))
    if not following:
        return
    return following
...
#print(retrieve_followers(78011))